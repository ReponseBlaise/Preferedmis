  ...
  const handleReply = async (parentId, replyText) => {
    ...
    try {
      ...
      await api.sendMessage({
        receiver_id:
          originalMessage.sender_id === user.id
                ...
                <div className="flex items-start gap-4">
                    ...
                    >
                      {activeTab === "received"
                        ? message.sender?.full_name?.charAt(0) ||
                          t("unknown").charAt(0)
                        : message.receiver?.full_name?.charAt(0) ||
                          t("unknown").charAt(0)}
                    </div>
                    {activeTab === "sent" && (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {replyMessages[message.id] &&
                      replyMessages[message.id].length > 0 && (
                        <div className="mt-2 flex items-center gap-2 text-xs text-blue-600 bg-blue-50 px-2 py-1 rounded-full w-fit">
                          <Reply size={12} />
                          <span>
                            {replyMessages[message.id].length} repl
                            {replyMessages[message.id].length > 1 ? "ies" : "y"}
                          </span>
                        </div>
                      )}
                  </div>
            ...
            <form onSubmit={handleSend} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                  placeholder={
                    t("subjectPlaceholder") || "Enter message subject"
                  }
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                  placeholder={
                    t("messagePlaceholder") || "Type your message here..."
                  }
                />
              </div>
      ...
      {selectedMessage && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 overflow-y-auto">
          <div className="bg-white rounded-lg p-6 w-full max-w-3xl my-8">
            <div className="flex justify-between items-start mb-4">
              <div>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span
                    className={`text-xs px-2 py-1 rounded-full ${getPriorityColor(selectedMessage.priority)}`}
                  >
            ...
            {/* Replies */}
            {replyMessages[selectedMessage.id] &&
              replyMessages[selectedMessage.id].length > 0 && (
                <div className="mt-4 pt-4 border-t">
                  <h4 className="font-medium text-gray-700 mb-3">
                    {t("replies") || "Replies"} (
                    {replyMessages[selectedMessage.id].length})
                  </h4>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              <p className="text-sm font-medium">
                                {reply.sender?.full_name || t("unknown")}
                              </p>
                              <p className="text-xs text-gray-500">
