...
const Users = () => {
  ...
  const handleDelete = async (id) => {
    ...
    if (
      !window.confirm(
        `Are you sure you want to delete user "${userToDelete?.full_name}"?\n\nNote: If the user has associated data (messages, attendance records, etc.), they will be deactivated instead of deleted to preserve data integrity.`,
      )
    )
      return;
  ...
  const handleEdit = (user) => {
    ...
    api
      .get(`/auth/users/${user.id}/projects`)
      .then((response) => {
      ...
      <div className="md:hidden space-y-3">
        {users.map((user) => {
          ...
          return (
            <div
              key={user.id}
              className={`card ${isDeleted ? "opacity-60" : ""}`}
            >
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {users.map((user) => {
              ...
              return (
                <tr
                  key={user.id}
                  className={`hover:bg-gray-50 ${isDeleted ? "bg-gray-100 opacity-70" : ""}`}
                >
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div
                      className={`font-medium ${isDeleted ? "text-gray-500 line-through" : "text-gray-900"}`}
                    >
                      {isDeleted
                        ? user.full_name.replace(
                            /\[DELETED\] [a-f0-9-]+\s*/,
                            "[DELETED] ",
                          )
                        : user.full_name}
                  ...
                  <td
                    className={`px-6 py-4 whitespace-nowrap text-sm ${isDeleted ? "text-gray-400" : "text-gray-600"}`}
                  >
                    {user.email}
                  </td>
                  <td
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        title={
                          isDeleted
                            ? "Cannot change status of deleted user"
                            : user.is_active
                              ? "Deactivate"
                              : "Activate"
                        }
