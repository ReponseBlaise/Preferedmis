...
import { reportAPI, projectAPI, attendanceAPI, inventoryAPI } from '../services/api';
import { useAuth } from '../contexts/AuthContext';
import { useTheme } from '../contexts/ThemeContext';
import { useTranslation } from 'react-i18next';
import { Download, Calendar, Filter, BarChart3, Users, Package, DollarSign } from 'lucide-react';
...
const Reports = () => {
  ...
  const handleExportPayroll = async () => {
    try {
      ...
      link.href = url;
      link.setAttribute('download', `payroll_report_${startDate}_to_${endDate}.xlsx`);
  ...
  const handleExportInventory = async () => {
    try {
      ...
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `inventory_report_${startDate}_to_${endDate}.xlsx`);
      document.body.appendChild(link);
      link.click();
      link.parentElement.removeChild(link);
      window.URL.revokeObjectURL(url);
    ...
    <div className={`min-h-screen ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-900'} p-6`}>
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          {/* Payroll Report */}
          <div className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow-md p-6 border-l-4 border-blue-600`}>
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center gap-3">
                <Users className="w-8 h-8 text-blue-600" />
                <div>
                  <h3 className="text-lg font-semibold">{t('payrollReport')}</h3>
            ...
            {reportData.payroll && (
              <div className={`mb-4 p-3 rounded ${isDarkMode ? 'bg-gray-700' : 'bg-gray-100'}`}>
                <p className="text-sm">Total Workers: {reportData.payroll.length || 0}</p>
                {reportData.payroll.length > 0 && (
                  <p className="text-sm">Total Payroll: {reportData.payroll.reduce((sum, w) => sum + (w.total_amount || 0), 0).toLocaleString()} RWF</p>
                )}
              </div>
            )}
          ...
          {/* Inventory Report */}
          <div className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow-md p-6 border-l-4 border-green-600`}>
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center gap-3">
            ...
            {reportData.inventory && (
              <div className={`mb-4 p-3 rounded ${isDarkMode ? 'bg-gray-700' : 'bg-gray-100'}`}>
                <p className="text-sm">Total Items: {reportData.inventory.length || 0}</p>
                {reportData.inventory.length > 0 && (
                  <p className="text-sm">Total Value: {reportData.inventory.reduce((sum, item) => sum + (item.total_value || 0), 0).toLocaleString()} RWF</p>
                )}
              </div>
            )}
          ...
          <div className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow-md p-6`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className={`p-4 rounded ${isDarkMode ? 'bg-gray-700' : 'bg-gray-100'}`}>
                ...
                <p className="text-2xl font-bold">{reportData.attendance.length}</p>
              </div>
              <div className={`p-4 rounded ${isDarkMode ? 'bg-gray-700' : 'bg-gray-100'}`}>
                <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Unique Workers</p>
                <p className="text-2xl font-bold">{new Set(reportData.attendance.map(a => a.worker_id)).size}</p>
              </div>
              <div className={`p-4 rounded ${isDarkMode ? 'bg-gray-700' : 'bg-gray-100'}`}>
