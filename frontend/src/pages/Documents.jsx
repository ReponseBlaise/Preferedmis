  ...
  const filterDocuments = () => {
    ...
    if (activeTab === "shared") {
      filtered = filtered.filter(
        (d) => d.visibility === "shared" || d.visibility === "public",
      );
    } else {
  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto">
        ...
        {/* Filters */}
        <div
          className={`${isDarkMode ? "bg-gray-800" : "bg-white"} rounded-lg shadow-md p-4 mb-6`}
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredDocuments.map((doc) => (
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      <span
                        className={`text-xs px-2 py-1 rounded ${isDarkMode ? "bg-gray-700" : "bg-gray-100"}`}
                      >
                        {t(doc.category)}
                      </span>
                      <span
                        className={`text-xs px-2 py-1 rounded flex items-center gap-1 ${isDarkMode ? "bg-gray-700" : "bg-gray-100"}`}
                      >
                ...
                {doc.description && (
                  <p
                    className={`text-sm mb-3 line-clamp-2 ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}
                  >
                    {doc.description}
                  </p>
                )}
                ...
                <p
                  className={`text-xs mb-4 ${isDarkMode ? "text-gray-500" : "text-gray-500"}`}
                >
                  {t("by")}{" "}
                  {doc.created_by === user?.id
                    ? "You"
                    : doc.user?.full_name || "Unknown"}
                </p>
                ...
                {allUsers.map(
                  (u) =>
                    u.id !== user?.id && (
                      <label
                        key={u.id}
                        className={`flex items-center p-2 rounded ${isDarkMode ? "hover:bg-gray-700" : "hover:bg-gray-100"}`}
                      >
                        <input
                          type="checkbox"
