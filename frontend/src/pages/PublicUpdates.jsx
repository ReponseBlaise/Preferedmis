      ...
      <div className="card">
        <div className="flex flex-col sm:flex-row gap-4 mb-4">
          ...
          <div className="flex-1 flex flex-wrap gap-2">
            {["all", "announcement", "update", "alert", "milestone"].map(
              (type) => (
                <button
                  key={type}
                  ...
                  <div className="flex items-start gap-3 md:gap-4 flex-1">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap items-center gap-2 mb-3">
                        <span
                          className={`text-xs px-2 py-1 rounded-full ${getPriorityBadge(update.priority)}`}
                        >
                          {t(update.priority)}
                        </span>
                        <span
                          className={`text-xs px-2 py-1 rounded-full ${getTypeBadge(update.type)}`}
                        >
                          {t(update.type)}
                        </span>
                        {update.author && (
                      ...
                      {update.expires_at && (
                        <p className="text-xs text-gray-500 mt-3 flex items-center gap-1">
                          <Clock size={12} />
                          {t("expires")}:{" "}
                          {new Date(update.expires_at).toLocaleDateString()}
                        </p>
                      )}
                    </div>
                  </div>
