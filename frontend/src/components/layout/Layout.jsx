...
import {
  ...
  Bell, Megaphone, TrendingUp, FileText, BarChart3, Moon, Sun, File
} from 'lucide-react';
...
const Layout = () => {
  ...
  const menuItems = [
    ...
    { path: '/documents', icon: File, label: t('documents'), roles: ['manager', 'employee', 'storeman'] },
  ...
  return (
    <div className={`flex h-screen ${isDarkMode ? 'bg-gray-900' : 'bg-gray-100'}`}>
      ...
      <aside className={`${sidebarOpen ? 'translate-x-0' : '-translate-x-full'
        ...
        <div className={`p-4 flex items-center justify-between border-b ${isDarkMode ? 'border-gray-700' : 'border-primary-600'}`}>
          ...
          {filteredMenu.map((item) => (
            <Link
              ...
              className={`flex items-center gap-4 px-4 py-3 transition-colors ${isActive(item.path)
                  ? isDarkMode ? 'bg-gray-700 border-l-4 border-blue-400' : 'bg-primary-800 border-l-4 border-secondary-500'
                  : isDarkMode ? 'hover:bg-gray-700' : 'hover:bg-primary-600'
                }`}
            >
              <item.icon size={20} />
        ...
        <div className={`p-4 border-t ${isDarkMode ? 'border-gray-700' : 'border-primary-600'}`}>
          <div className="text-sm">
            <p className="font-semibold truncate">{user?.full_name}</p>
            <p className={`text-xs truncate ${isDarkMode ? 'text-gray-400' : 'text-primary-200'}`}>{user?.email}</p>
            <span className={`inline-block mt-2 px-2 py-1 rounded text-xs ${isDarkMode ? 'bg-gray-700' : 'bg-primary-800'}`}>
              {user?.role}
            </span>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className={`${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white'} shadow-sm border-b`}>
          <div className={`flex items-center justify-between px-4 py-3 ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>
        ...
        {/* Page Content */}
        <main className={`flex-1 overflow-y-auto p-4 md:p-6 ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
          <Outlet />
        </main>

        {/* Footer */}
        <footer className={`border-t ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} px-4 py-2`}>
