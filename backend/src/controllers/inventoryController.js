  ...
  try {
    ...
    if (!project_id) {
      return res
        .status(400)
        .json({
          error: "Please select a project before adding inventory items",
        });
    }
    if (!name) {
    ...
    if (!quantity || quantity <= 0) {
      return res
        .status(400)
        .json({ error: "Please enter a valid quantity (greater than 0)" });
    }
    if (!unit_price || unit_price <= 0) {
      return res
        .status(400)
        .json({ error: "Please enter a valid unit price (greater than 0)" });
    }
    ...
    if (error) {
      ...
      return res
        ...
        .json({
          error:
            "Failed to add inventory item. Please check all fields are correct.",
        });
    }
...
exports.getItems = async (req, res) => {
  try {
    ...
    if (itemIds.length > 0) {
      ...
      (movements || []).forEach((m) => {
        if (!movementsMap[m.inventory_item_id])
          movementsMap[m.inventory_item_id] = { in: 0, out: 0 };
        movementsMap[m.inventory_item_id][m.movement_type] += parseFloat(
          m.quantity || 0,
        );
      });
    }
...
exports.updateItem = async (req, res) => {
  try {
    const { name, description, quantity, unit, unit_price, category_id } =
      req.body;
    const updates = {};
...
exports.getInventoryReport = async (req, res) => {
  try {
    ...
    const totalSpent = categoryArray.reduce(
