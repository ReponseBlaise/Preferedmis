...
exports.sendMessage = async (req, res) => {
  try {
    ...
    if (!receiver_id) return res.status(400).json({ error: 'Please select a recipient' });
    if (!subject?.trim()) return res.status(400).json({ error: 'Message subject is required' });
    if (!message?.trim()) return res.status(400).json({ error: 'Message content is required' });

    let replySubject = subject;
    if (parent_id) {
      const { data: parentMessage } = await supabaseAdmin.from('messages').select('subject').eq('id', parent_id).single();
      if (parentMessage) {
    ...
    try {
      const { data: receiverData } = await supabaseAdmin.from('users').select('email').eq('id', receiver_id).single();
      if (receiverData?.email) {
        ...
        emailService.sendEmail(receiverData.email, `${parent_id ? 'Re: ' : 'New Message: '}${replySubject}`, emailBody).catch(() => {});
      }
    } catch {}
...
exports.editMessage = async (req, res) => {
  try {
    ...
    const { data: existing } = await supabaseAdmin.from('messages').select('sender_id').eq('id', id).single();
...
exports.deleteMessage = async (req, res) => {
  try {
    ...
    await supabaseAdmin.from('message_attachments').delete().eq('message_id', id);
...
exports.forwardMessage = async (req, res) => {
  try {
    ...
    if (!receiver_id) return res.status(400).json({ error: 'Recipient is required' });

    const { data: orig } = await supabaseAdmin.from('messages').select('*').eq('id', id).single();
